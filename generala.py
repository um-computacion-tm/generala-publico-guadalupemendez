...
def buscar_full(dados, repetidos, cantidad_repetidos, estricto=False):
    encontre = False
    for repetido in repetidos:
        if repetido == cantidad_repetidos:
            encontre = True
...
def calcular_puntos(numero_lanzamiento, dados, juego):
    ...
    if juego == "generala":
        ...
        repetidos = calcular_repetidos(dados)
        if buscar_full(dados, repetidos, 3) and buscar_full(dados, repetidos, 2):
            puntos = 30
            if numero_lanzamiento == 1:
                puntos += 5
    else:
...
def main():
    ...
    while juego.esta_jugado:
        if juego.victoria():
            print("Generala servida, ganaste")
            break
        else:
            ...
            print('Jugador actual: {}'.format(juego.jugador_actual))
            print(juego.turno_actual.dados_finales)
            jugada = input('Indique la jugada que quiere anotar\n')
